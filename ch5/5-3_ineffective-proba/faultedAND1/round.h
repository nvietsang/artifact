...
forceinline void ROUND(ascon_state_t* s, uint64_t C, int f) {
  ...
  s->b[1][0] ^= s->b[3][0] & ~s->b[2][0];
  s->b[1][1] ^= s->b[3][1] & ~s->b[2][1];
  s->b[1][2] ^= s->b[3][2] & ~s->b[2][2];
  s->b[1][3] ^= s->b[3][3] & ~s->b[2][3];
  s->b[1][4] ^= s->b[3][4] & ~s->b[2][4];
  s->b[1][5] ^= s->b[3][5] & ~s->b[2][5];
  s->b[1][6] ^= s->b[3][6] & ~s->b[2][6];
  s->b[1][7] ^= s->b[3][7] & ~s->b[2][7]; 
