...
static const uint32_t Rround_constants[10] =
{
...
static const uint8_t plaintexts[3][16] = {
    {0x2c, 0x81, 0x4b, 0x66, 0x21, 0x42, 0x3f, 0xfd, 0x94, 0x21, 0x78, 0xa2, 0xfe, 0x33, 0x8f, 0xf4},
    {0x0b, 0x6a, 0xe3, 0xed, 0x4d, 0x90, 0xda, 0xd9, 0xe8, 0xa1, 0xe5, 0xad, 0xef, 0x40, 0x05, 0x12},
    {0x7e, 0x1c, 0xa7, 0x1c, 0x7c, 0xa3, 0xbf, 0x7a, 0xd7, 0xd9, 0xd3, 0xc4, 0x1c, 0x00, 0xaf, 0x70}
};
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("Faulted Rcon:\n");
        for (i = 0; i < 10; i++) printf("%2d ", i); printf("\n");
        for (i = 0; i < 10; i++) printf("---"); printf("\n");
        for (i = 0; i < 10; i++){
            printf("%02x ", round_constants[i]);
        }
        printf("\n");

        printf("Reference Rcon:\n");
        for (i = 0; i < 10; i++) printf("%2d ", i); printf("\n");
        for (i = 0; i < 10; i++) printf("---"); printf("\n");
        for (i = 0; i < 10; i++){
            printf("%02x ", Rround_constants[i]);
        }
        printf("\n");
    }
#endif
...
#ifdef INJECT_FAULT
    // Faulty ciphertexts
    FILE *file = fopen("outputs/fcpts.txt", "w");
#else
    // Correct ciphertexts
    FILE *file = fopen("outputs/ccpts.txt", "w");
#endif
    if (file == NULL){
    ...
    for (i = 0; i < N; i++){
        for (j = 0; j < 16; j++) buf[j] = plaintexts[i][j];
        ret = mbedtls_aes_crypt_ecb(&ctx, mode, buf, buf);
        if (ret != 0) {
            printf("[FAILED] ECB encryption!\n");
            return ret;
